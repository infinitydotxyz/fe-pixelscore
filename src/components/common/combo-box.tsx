...
export const ComboBox = <T extends ComboBoxBaseType>({ options, value, onChange }: Props<T>) => {
  ...
  if (query !== '') {
    filteredPeople = options.filter((v: T) => v.name.toLowerCase().includes(query.toLowerCase()));
  }
                  ...
                  <Combobox.Option
                    ...
                    className={({ active }) =>
                      `cursor-default select-none relative py-2 pl-10 pr-4 ${
                        active ? 'dark:text-dark-body text-light-body bg-black' : 'text-gray-900'
                      }`
                    }
                    value={person}
                  >
                        ...
                        {selected ? (
                          <span
                            className={`absolute inset-y-0 left-0 flex items-center pl-3 ${
                              active ? 'dark:text-dark-body text-light-body' : 'text-black'
                            }`}
                          >
